...
bool MulNX::Messaging::MessageManager::Registe(ModuleBase* const Module) {
...
bool MulNX::Messaging::MessageManager::Subscribe(const MsgType MsgType, ModuleBase* const Module) {
	std::unique_lock lock(this->MyThreadMutex);
...
MulNXHandle MulNX::Messaging::MessageManager::CreateMessageChannel() {
	...
	MulNXHandle hChannel = this->MulNXi->IHandleSystem().CreateHandle();
	Channel->hChannel = hChannel;
	this->Channels[hChannel] = std::move(Channel);
	return hChannel;
}
MulNX::Messaging::IMessageChannel* MulNX::Messaging::MessageManager::GetMessageChannel(const MulNXHandle& hChannel) {
	std::unique_lock lock(this->MyThreadMutex);
...
bool MulNX::Messaging::MessageManager::Unsubscribe(const MsgType MsgType, ModuleBase* const Module) {
	std::unique_lock lock(this->MyThreadMutex);
...
bool MulNX::Messaging::MessageManager::Publish(Message&& Msg) {
	//先获取锁
	std::unique_lock lock(this->MyThreadMutex);
	//总线订阅
	bool HasMain = false;
...
bool MulNX::Messaging::MessageManager::Release() {
	//原子变量不需要锁
...
bool MulNX::Messaging::MessageManager::NextMsg() {
...
bool MulNX::Messaging::MessageManager::Init() {
	return true;
}
void MulNX::Messaging::MessageManager::ThreadMain() {
	if (this->NextMsg()) {
		//有消息在处理,快速轮询
...
void MulNX::Messaging::MessageManager::VirtualMain() {
