...
bool IPCer::GetWindowPathByName(const LPCWSTR& WindowName, std::filesystem::path& Output) {
...
std::filesystem::path IPCer::PathGet_CS2Resources() {
    return this->Paths.MulNX.CS2Resources.Path;
}
std::filesystem::path IPCer::PathGet_Tool_Cfg() {
...
std::filesystem::path IPCer::PathGet_CS_cfg() {
...
bool IPCer::GetFileNames(std::vector<std::string>& FileNames, const std::filesystem::path& FolderPath, const std::vector<std::string>& Filter, const bool Extension) {
    //检查路径存在且为文件夹
    if (!std::filesystem::exists(FolderPath) || !std::filesystem::is_directory(FolderPath)) {
        this->IDebugger->AddError("尝试获取指定文件夹内文件名时出现错误：目标文件夹不存在！\n文件夹路径：" + FolderPath.string());
        return false;
...
bool IPCer::FileDelete(const std::string& FileName, const std::filesystem::path& FolderPath) {
    //检查文件名是否为空
    if (FileName.empty()) {
...
bool IPCer::FileMove(const std::string& FileName, const std::filesystem::path& Resource, const std::filesystem::path& Target) {
    //检查文件名是否为空
    if (FileName.empty()) {
