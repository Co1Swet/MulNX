...
void SolutionManager::Init(MulNXiCore* MulNXi, CameraDrawer* CamDrawer, ElementManager* EManager, ProjectManager* PManager) {
    //核心服务
    this->MulNXi = MulNXi;
    //系统服务
...
void SolutionManager::Playing_Enable() {
    ...
    this->Playing = true;
    this->MulNXi->MulNXiGlobalVars().CampathPlaying = true;
    this->MulNXi->IDebugger().AddInfo("已开启播放");
}
void SolutionManager::Playing_Disable() {
    this->Playing = false;
    this->MulNXi->MulNXiGlobalVars().CampathPlaying = false;
