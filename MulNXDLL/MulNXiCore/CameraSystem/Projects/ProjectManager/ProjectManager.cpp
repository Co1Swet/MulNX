...
void ProjectManager::Init(MulNXiCore* MulNXi, ElementManager* EManager, SolutionManager* SManager) {
	//核心服务
	this->MulNXi = MulNXi;
	//系统服务
...
bool ProjectManager::Playing_AutoCall(const MulNXMessage& Msg) {
	...
	case MsgType::Game_RoundEnd: {
		...
		return this->SManager->Playing_SetSolution(OnEnd[temp], PlaybackMode::Serial);
	}
	}
	return false;
}