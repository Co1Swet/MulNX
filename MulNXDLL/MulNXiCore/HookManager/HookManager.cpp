...
void HookManager::ThreadMain() {
	...
	if (ReHook) {
		...
		ReHook = false;
		this->MulNXi->MulNXiGlobalVars().CoreReady = true;
	}
}

void HookManager::ProcessMsg(MulNXMessage* Msg) {
	switch (Msg->Type) {
...
LRESULT __stdcall HookManager::EntryMyWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	if (MulNXiCore::GetInstance().HookManager().MyWndProc(hwnd, uMsg, wParam, lParam)) {
		return true;
	}	
	return CallWindowProcW(MulNXiCore::GetInstance().HookManager().OriginWndProc, hwnd, uMsg, wParam, lParam);
}
...
HRESULT __stdcall HookManager::MyPresent(IDXGISwapChain* swapChain, UINT syncInterval, UINT flags) {
	if(!this->OriginWndProc)
		this->OriginWndProc = (WNDPROC)SetWindowLongPtrW(this->MulNXi->IUISystem().CS2hWnd, GWLP_WNDPROC, (LONG_PTR)MulNXiCore::GetInstance().HookManager().EntryMyWndProc);
	if (this->MulNXi->MulNXiGlobalVars().SystemReady) {
